...
$(function(){
	...
	tree.tree({
		onClick : function(easyUITree){
			if(tree.tree("isLeaf",easyUITree.targets)){
				datagrid.datagrid('reload', {
					categoryId :easyUITree.id
		        });
			}
		}
	});
...
var contentListToolbar = [{
    ...
    iconCls:'icon-add',
    handler:function(){
    	var easyUITree = $("#contentCategoryTree").tree("getSelected");
    	if(!easyUITree || !$("#contentCategoryTree").tree("isLeaf",easyUITree.targets)){
    ...
    handler:function(){
		...
		TT.createWindow({
			...
			onLoad : function(){
				...
				// 实现图片
				if(data.pic){
					$("#contentEditForm [name=pic]").after("<a href='"+data.pic+"' targets='_blank'><img src='"+data.pic+"' width='80' height='50'/></a>");
				}
				if(data.pic2){
					$("#contentEditForm [name=pic2]").after("<a href='"+data.pic2+"' targets='_blank'><img src='"+data.pic2+"' width='80' height='50'/></a>");
				}
